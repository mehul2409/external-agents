...
graph steps. **31/31 passing** across the whole tool, covering all four
required Curveball cases plus the regression lock:
...
| Parser fidelity | headings inside a message body are content, not events; a speaker turn spans every line to the next speaker |
...
| `f301f2d3f6c2` | `0922b5e` | **Closing state.** Final record: what was built, what each product contributes, and — stated plainly rather than omitted — what is still not done: protocol contracts invisible, per-run rather than per-finding completeness, verification confirming presence rather than the call, and a block path proven by unit test because real history contains no breaking + unmentioned + verified case. |
| `9460153f58dd` | `59150df` | **Adaptation under a format change (noon Curveball).** The reconstruction above was performed from checkpoint context *before* reading any code, and the graph impact analysis was run *before* editing — enumerating `analyze`, `renderMarkdown` and `renderText` as intent consumers and refuting `scoreFinding`, which does not exist (scoring is inline at `pr-impact.mjs:408-412`). |
...
- **Verification confirms presence, not the call.** It requires the symbol to
  appear as a whole identifier within a window around the citation. That
  rejects substring coincidences, but a symbol named in a comment or an
  unrelated declaration inside the window would still confirm. Parsing the
  call site would be stronger.
...
### Three defects the loop exposed, and the fixes

Running `act` end to end surfaced three real bugs that `analyze` alone had
hidden. Each is now covered by a test that **fails without its fix** (verified
by reverting the fix and re-running).

**1. Message-body headings were reported as unrecognised checkpoint events.**
`parseLegacyText` applied heading detection everywhere, so every `## Step 1`
a person typed became an "unrecognised checkpoint event" — 16 of them on a real
checkpoint, printed into the PR comment. This actively defeated the Curveball
requirement: the genuine unknown-event signal was drowned in false ones.
Headings are now structural only *before* the transcript rule; inside the
conversation they are content. **16 spurious events → 0.**

**2. ~78% of every transcript body was being discarded.** The labelled
boundary `── Transcript (checkpoint scope) ───` begins with **two** box
characters, and the boundary test required three — so it never matched, and
continuation lines were dropped. Only the first line of each turn survived.
On a real checkpoint this captured **5,213 of 23,930 characters**. Because the
intent haystack is what "was this consumer mentioned?" searches, the missing
78% biased the tool toward false *"never mentioned"* verdicts — the wrong
direction for a safety tool, and precisely the class of error the Curveball
work existed to eliminate. A turn now spans every line to the next speaker:
**21,726 / 23,930 characters captured (22% → 91%).**

**3. The verifier confirmed symbols that do not exist.** `verifyFinding` used
a bare substring test, so `notif` "verified" against `notifyConsumers` and
returned `verified: true`. Since verification is the only thing standing
between a graph citation and a blocked merge, a permissive verifier undermines
the entire gate. Matching now requires an identifier boundary.

The common thread is that all three are **silent** failures — nothing threw,
nothing was logged, and `analyze` output looked reasonable. They became visible
only when the tool was asked to *act* on its own data, which is an argument for
closing the loop beyond the feature itself: an automated decision forces a
standard of evidence that a human-read report does not.
